...
void c_Work_Remote::Start_Cmd(QJsonObject object)
{
	...
	if (Cmd_Name == "Work_Start") {
		emit Status("生成巡检任务" + QString(QJsonDocument(object).toJson()));
...
void c_Work_Remote::Step_3(QString Carbox_Num, QString Bogie_Num, QString Axis_Num)
{
	...
	m_Axis_Num = "1";
	Init();
	emit Status("巡检任务完成，返回充电位置，检查充电状态");
	m_Work_Start = false;
}
/*************************************************************************************************************************************************
**精扫相机执行情况
...
void c_Work_Remote::System_Scan(QJsonObject db)
{
	...
	if (m_Jaka_120_Monitor_Connected && m_Jaka_121_Monitor_Connected)
	{
		...
		if (m_State_19 && m_State_21) { m_State_23 = true; }else { m_State_23 = false; }//上电状态更新
		if (m_State_20 && m_State_22) { m_State_24 = true; }else { m_State_24 = false; }//使能状态更新	
	}
...
void c_Work_Remote::State_2_Loop()
{
	...
	emit Fuction_7();
	emit Status("主动力位置清零中");
	loop.exec();
	emit Status("主动力位置清零完成");
}
//主动力故障复位（线圈，工控机立即复位）
void c_Work_Remote::State_3_Loop()
{
	...
	emit Fuction_8();
	emit Status("主动力故障复位中");
	loop.exec();
	emit Status("主动力故障复位完成");
}
//正向连续运行（线圈，工控机立即复位）
void c_Work_Remote::State_4_Loop()
{
	...
	emit Fuction_11();
	emit Status("正向连续运行开始");
	loop.exec();
	emit Status("正向连续运行完成");
}
//返向连续运行（线圈，工控机立即复位）
void c_Work_Remote::State_5_Loop()
{
	...
	emit Fuction_12();
	emit Status("返向连续运行开始");
	loop.exec();
	emit Status("返向连续运行完成");
}
...
void c_Work_Remote::State_6_Loop()
{
	...
	QObject::connect(this, &c_Work_Remote::is_State_15, &loop, &QEventLoop::quit);
	emit Fuction_13();
	emit Status("返向连续运行再启动开始");
	loop.exec();
	emit Status("返向连续运行再启动完成");
}
//机器人开机（线圈，工控机立即复位）
...
void c_Work_Remote::State_18_Loop()
{
	...
	emit Fuction_14();
	emit Status("面阵雷达正向运行检查到障碍");
	m_State_29 = true;
	loop.exec();
	m_State_29 = false;
	emit Status("面阵雷达正向运行检查到障碍解除");
   // emit Fuction_11();
}
//反向障碍停车
